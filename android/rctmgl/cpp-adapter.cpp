...
Java_com_mapbox_rctmgl_modules_RCTMGLModule_nativeInit(JNIEnv *env, jobject callingObject, jstring mqttHost, jint mqttPort)
{
  ...
  core->registerGeojsonCallback(Caches::FeatureCollectionType::FC_MATCHED_GREEN, [](std::string geojson)
                                { sendGeoJSONToJava(Caches::FeatureCollectionType::FC_MATCHED_GREEN, geojson); });
  core->registerGeojsonCallback(Caches::FeatureCollectionType::FC_MATCHED_RED, [](std::string geojson)
                                { sendGeoJSONToJava(Caches::FeatureCollectionType::FC_MATCHED_RED, geojson); });
  core->registerGeojsonCallback(Caches::FeatureCollectionType::FC_MATCHED_YELLOW, [](std::string geojson)
                                { sendGeoJSONToJava(Caches::FeatureCollectionType::FC_MATCHED_YELLOW, geojson); });
...
extern "C" JNIEXPORT void JNICALL
Java_com_mapbox_rctmgl_modules_RCTMGLModule_nativeUseMatchedLanes(JNIEnv *env, jclass clazz, jboolean use)
{
  V2X::Settings settings{static_cast<bool>(use)};
